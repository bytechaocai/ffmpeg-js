...
可以运行的代码以`main`为准，`dev`为开发代码，我用过几次没出现问题后，就会合并到`main`分支。
...
程序未做版本兼容测试，大家在使用过程中，如果没遇到问题，那就说明程序兼容该版本，如果遇到问题，那就说明不兼容。注意，这里的不兼容指的是node, ffmpeg, 7zip在执行过程中明确提示不支持或者报错，而该报错说明当前版本不支持程序功能。

## 使用方式

假设你要转码的视频目录是`c:/path/of/video`，代码目录是`c:/path/of/code`，则按以下顺序运行命令，js在代码目录运行，bat在视频目录运行。执行bat脚本需要运行`chcp 65001`将命令行编码改为`utf-8`，否则会导致ffmpeg无法识别包含中文的文件。
...
- `node ffmpeg.js c:/path/of/video [baseBitRate]`：在视频目录生成`3_ffmpeg.bat`并将码率相关数据写入`data.json`。
- `node preview.js c:/path/of/video [compressRatioThreshold]`：生成`preview.html`，展示压缩比。
- `2_rename.bat`：重命名文件，在文件名后加下划线。
- `3_ffmpeg.bat`：开始转码，此时只需要等待转码完成或报错就行，视频多总时长多文件大的情况下建议挂机或者睡觉。
...
`preview.js`可以在转码前运行也可以在转码后运行，当视频目录里没有`data.txt`时视作没有开始转码，此时展示的是预期码率和压缩率，否则展示实际结果。`preview.js`生成以下格式的html：
...
没有转码时，压缩率用码率计算，转码后，压缩率用文件大小计算。**使用码率计算的压缩率只用来参考，实际压缩率应该以文件大小为准**。

所有生成的脚本除了`1_meta.bat`会将日志（结果）输到文件外，其余脚本都不会将日志输出到文件。所有生成的bat脚本都在`c:/path/of/video`下直接执行，`3_ffmpeg.bat`有可选参数基准码率，后文会介绍，`5_7zip.bat`需要密码参数，比如`5_7zip.bat 123456`，123456就是密码，`preview.js`有一个可选参数压缩率阈值，除此以外其余脚本都不需要参数。
...
这个json就是码率参考，用来处理同目录下不同尺寸不同尺寸的文件，基准码率就是1080p的码率，然后用基准码率乘以倍率magnification就是对应分辨率的码率。如果视频大小不是标准大小，则使用最近的码率。如果没有加码率参数，则由ffmpeg自己决定分辨率，此时，`5_7zip.bat`会变成`ffmpeg -hide_banner -y -i "input video_.mp4" -c:a aac -c:v av1_nvenc "input video.mp4"`。

上面的倍率是我问ai得到的，目前我没有一个很好的方法来根据分辨率来计算能保留画质的最小码率，只能使用分段函数做简单计算。你可以根据实际情况来修改上面的json文件。
